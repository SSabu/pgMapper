...
app.get('/map', function(req, res, next) {
  ...
  var config = "postgres://sandeepsabu:Spi9dlee6@ssdbinstance.cs3dgspl0uzj.us-west-1.rds.amazonaws.com:5432/sfparks";
  ...
  client.connect(function(err, client) {
    ...
    client.query("SELECT ST_AsGeoJSON(wkb_geometry) FROM sfparks;", function(err, result, done) {
      client.end();
      if (err) {
...
app.post('/map', function(req, res, next) {
  ...
  var nearestQuery = "SELECT ST_AsGeoJSON(wkb_geometry) FROM sfparks ORDER BY wkb_geometry <-> ST_SetSRID(ST_MakePoint("+lon+","+lat+"), 4326) LIMIT 5;";
